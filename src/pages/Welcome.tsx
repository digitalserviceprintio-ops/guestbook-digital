    ...
    <div className="min-h-screen bg-background overflow-hidden">
      ...
      <div className="relative h-[70vh] min-h-[480px]">
        ...
        <div className="relative z-10 h-full flex flex-col items-center justify-end pb-10 px-5 text-center">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="space-y-3">
            <p className="text-xs font-body tracking-[0.3em] uppercase text-primary-foreground/80">The Wedding of</p>
      ...
      <main className="max-w-md mx-auto px-5 py-8 space-y-6">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 }} className="text-center space-y-2">
          <p className="text-xs font-body text-muted-foreground tracking-widest uppercase">Bismillahirrahmanirrahim</p>
          <p className="text-sm font-body text-muted-foreground leading-relaxed">{settings.invitationText}</p>
        </motion.div>

        {/* Event Details */}
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.6 }} className="rounded-2xl bg-card p-6 shadow-elevated space-y-4">
          <h2 className="font-display text-lg font-bold text-card-foreground text-center">Detail Acara</h2>
          <div className="space-y-3">
            <div className="flex items-start gap-3">
              ...
              <div>
                <p className="text-sm font-body font-semibold text-card-foreground">{formatDate(settings.eventDate)}</p>
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                <p className="text-sm font-body font-semibold text-card-foreground">{settings.akadTime} - {settings.endTime} WIB</p>
                <p className="text-xs font-body text-muted-foreground">Akad: {settings.akadTime} · Resepsi: {settings.resepsiTime}</p>
              </div>
            </div>
            <div className="flex items-start gap-3">
              <div className="h-9 w-9 rounded-xl gradient-gold flex items-center justify-center shrink-0">
              ...
              <div>
                <p className="text-sm font-body font-semibold text-card-foreground">{settings.venueName}</p>
                <p className="text-xs font-body text-muted-foreground">{settings.venueAddress}</p>
              </div>
            </div>
          </div>
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.7 }} className="space-y-3">
          {settings.rsvpOpen ? (
            ...
            <div className="w-full bg-muted text-muted-foreground font-body font-medium py-4 rounded-xl text-center text-sm">
              RSVP Sudah Ditutup
            </div>
          )}
          <button
        ...
        <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.9 }} className="text-center text-[11px] font-body text-muted-foreground">
