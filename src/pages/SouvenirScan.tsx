    ...
    <div className="min-h-screen bg-background">
      ...
      >
        <div className="flex items-center gap-3 max-w-lg mx-auto">
          ...
          <h1 className="font-display text-lg font-bold text-foreground">Verifikasi Souvenir</h1>
        </div>
      </motion.header>
      ...
      <main className="max-w-lg mx-auto px-5 py-8">
        {loading ? (
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="rounded-2xl bg-card p-6 shadow-elevated text-center space-y-3">
            <XCircle className="h-12 w-12 text-destructive mx-auto" />
            <h2 className="font-display text-lg font-bold text-card-foreground">Tamu Tidak Ditemukan</h2>
            <p className="text-sm font-body text-muted-foreground">QR Code tidak valid atau tamu tidak terdaftar.</p>
          </motion.div>
        ) : guest.souvenir_picked_up ? (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="rounded-2xl bg-card p-6 shadow-elevated text-center space-y-3">
            <CheckCircle2 className="h-12 w-12 text-success mx-auto" />
            <h2 className="font-display text-lg font-bold text-card-foreground">{guest.name}</h2>
            <p className="text-sm font-body text-muted-foreground">Souvenir sudah diambil sebelumnya.</p>
          </motion.div>
        ) : guest.status !== "hadir" ? (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="rounded-2xl bg-card p-6 shadow-elevated text-center space-y-3">
          ...
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="rounded-2xl bg-card p-6 shadow-elevated text-center space-y-4">
            ...
            <p className="text-sm font-body text-muted-foreground">{guest.number_of_guests} orang</p>
            <button
              onClick={handlePickup}
              disabled={processing}
              className="w-full gradient-gold text-primary-foreground font-body font-semibold py-3.5 rounded-xl shadow-elevated hover:opacity-90 transition-opacity text-sm disabled:opacity-50"
            >
